...
namespace Kinect
{
	...
	public static int NuiImageStreamOpen(NuiImageType eImageType, NuiImageResolution eResolution, uint dwImageFrameFlags_NotUsed, uint dwFrameLimit, IntPtr hNextFrameEvent, ref IntPtr phStreamHandle)
	{
        ...
        public static bool PollSkeleton(ref NuiTransformSmoothParameters smoothParameters, ref NuiSkeletonFrame skeletonFrame)
        {
            ...
            int hr = Wrapper.NuiSkeletonGetNextFrame(0, ref skeletonFrame);
            if (hr == 0)
            {
                newSkeleton = true;
            }
            ...
            if (newSkeleton)
            {
                hr = Wrapper.NuiTransformSmooth(ref skeletonFrame, ref smoothParameters);
                if (hr != 0)
                {
		...
		if (newColor)
		{
            ...
            int hr = Wrapper.NuiImageStreamGetNextFrame(colorStreamHandle, 0, ref imageFramePtr);
            if (hr == 0)
            {
                newColor = true;
                ...
                hr = Wrapper.NuiImageStreamReleaseFrame(colorStreamHandle, imageFramePtr);
            }
#endif
		...
		bool newDepth = GetDepthFrameData(depthPlayerData, ref depthBufLen, true);
            ...
            if (isNearMode)
            {
                Wrapper.NuiImageStreamSetImageFrameFlags(depthStreamHandle, NuiImageStreamFlags.EnableNearMode);
            }
            else
            {
                Wrapper.NuiImageStreamSetImageFrameFlags(depthStreamHandle, NuiImageStreamFlags.None);
            }

            int hr = Wrapper.NuiImageStreamGetNextFrame(depthStreamHandle, 0, ref imageFramePtr);
            if (hr == 0)
            {
                newDepth = true;
                ...
                frameTexture.UnlockRect(0);
                hr = Wrapper.NuiImageStreamReleaseFrame(depthStreamHandle, imageFramePtr);
            }
#endif
