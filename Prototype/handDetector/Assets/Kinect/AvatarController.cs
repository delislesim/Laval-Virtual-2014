...
namespace Kinect
{
    ...
    public class AvatarController : MonoBehaviour
    {
        ...
        public void Start()
        {
            ...
            RotateToCalibrationPose(0, Manager.IsCalibrationNeeded());
        }
        ...
        public void UpdateAvatar(uint UserID, bool IsNearMode)
        {
            ...
            TransformBone(UserID, Wrapper.SkeletonJoint.HIPS, 1, flipJoint);
            TransformBone(UserID, Wrapper.SkeletonJoint.SPINE, 2, flipJoint);
            TransformBone(UserID, Wrapper.SkeletonJoint.NECK, 3, flipJoint);
            TransformBone(UserID, Wrapper.SkeletonJoint.HEAD, 4, flipJoint);

            // Beyond this, switch the arms and legs.

            // Left Arm --> Right Arm
            TransformBone(UserID, Wrapper.SkeletonJoint.LEFT_COLLAR, !MirroredMovement ? 5 : 11, flipJoint);
            TransformBone(UserID, Wrapper.SkeletonJoint.LEFT_SHOULDER, !MirroredMovement ? 6 : 12, flipJoint);
            TransformBone(UserID, Wrapper.SkeletonJoint.LEFT_ELBOW, !MirroredMovement ? 7 : 13, flipJoint);
            TransformBone(UserID, Wrapper.SkeletonJoint.LEFT_WRIST, !MirroredMovement ? 8 : 14, flipJoint);
            TransformBone(UserID, Wrapper.SkeletonJoint.LEFT_HAND, !MirroredMovement ? 9 : 15, flipJoint);
            TransformBone(UserID, Wrapper.SkeletonJoint.LEFT_FINGERTIP, !MirroredMovement ? 10 : 16, flipJoint);

            // Right Arm --> Left Arm
            TransformBone(UserID, Wrapper.SkeletonJoint.RIGHT_COLLAR, !MirroredMovement ? 11 : 5, flipJoint);
            TransformBone(UserID, Wrapper.SkeletonJoint.RIGHT_SHOULDER, !MirroredMovement ? 12 : 6, flipJoint);
            TransformBone(UserID, Wrapper.SkeletonJoint.RIGHT_ELBOW, !MirroredMovement ? 13 : 7, flipJoint);
            TransformBone(UserID, Wrapper.SkeletonJoint.RIGHT_WRIST, !MirroredMovement ? 14 : 8, flipJoint);
            TransformBone(UserID, Wrapper.SkeletonJoint.RIGHT_HAND, !MirroredMovement ? 15 : 9, flipJoint);
            TransformBone(UserID, Wrapper.SkeletonJoint.RIGHT_FINGERTIP, !MirroredMovement ? 16 : 10, flipJoint);

            if (!IsNearMode)
            {
                // Left Leg --> Right Leg
                TransformBone(UserID, Wrapper.SkeletonJoint.LEFT_HIP, !MirroredMovement ? 17 : 21, flipJoint);
                TransformBone(UserID, Wrapper.SkeletonJoint.LEFT_KNEE, !MirroredMovement ? 18 : 22, flipJoint);
                TransformBone(UserID, Wrapper.SkeletonJoint.LEFT_ANKLE, !MirroredMovement ? 19 : 23, flipJoint);
                TransformBone(UserID, Wrapper.SkeletonJoint.LEFT_FOOT, !MirroredMovement ? 20 : 24, flipJoint);

                // Right Leg --> Left Leg
                TransformBone(UserID, Wrapper.SkeletonJoint.RIGHT_HIP, !MirroredMovement ? 21 : 17, flipJoint);
                TransformBone(UserID, Wrapper.SkeletonJoint.RIGHT_KNEE, !MirroredMovement ? 22 : 18, flipJoint);
                TransformBone(UserID, Wrapper.SkeletonJoint.RIGHT_ANKLE, !MirroredMovement ? 23 : 19, flipJoint);
                TransformBone(UserID, Wrapper.SkeletonJoint.RIGHT_FOOT, !MirroredMovement ? 24 : 20, flipJoint);
            }
        ...
        public void RotateToCalibrationPose(uint userId, bool needCalibration)
        {
            ...
            if (needCalibration)
            {
                ...
                if (RightElbow != null)
                    RightElbow.rotation = Quaternion.Euler(0, -90, 90) *
                        initialRotations[(int)Wrapper.SkeletonJoint.RIGHT_ELBOW];

                // Left Elbow
                if (LeftElbow != null)
                    LeftElbow.rotation = Quaternion.Euler(0, 90, -90) *
                        initialRotations[(int)Wrapper.SkeletonJoint.LEFT_ELBOW];
            ...
            {
                // clear gesture info
                Manager.Instance.ClearGestures(userId);
        ...
        public void GestureInProgress(uint userId, Wrapper.Gestures gesture, float progress,
            Wrapper.SkeletonJoint joint, Vector3 screenPos)
        {
            //GestureInfo.guiText.text = string.Format("{0} Progress: {1:F1}%", gesture, (progress * 100));
            if ((gesture == Wrapper.Gestures.RightHandCursor || gesture == Wrapper.Gestures.LeftHandCursor) && progress > 0.5f)
            {
                //			if(HandCursor != null)
                //			{
            ...
            else if (gesture == Wrapper.Gestures.Click && progress > 0.3f)
            {
            ...
            else if ((gesture == Wrapper.Gestures.ZoomOut || gesture == Wrapper.Gestures.ZoomIn) && progress > 0.5f)
            {
            ...
            else if (gesture == Wrapper.Gestures.Wheel && progress > 0.5f)
            {
        ...
        public bool GestureComplete(uint userId, Wrapper.Gestures gesture,
            Wrapper.SkeletonJoint joint, Vector3 screenPos)
        {
            string sGestureText = gesture + " detected";
            if (gesture == Wrapper.Gestures.Click)
        ...
        public bool GestureCancelled(uint userId, Wrapper.Gestures gesture,
            Wrapper.SkeletonJoint joint)
        {
        ...
        void TransformBone(uint userId, Wrapper.SkeletonJoint joint, int boneIndex, bool flip)
        {
            ...
            Quaternion jointRotation = Manager.Instance.GetJointOrientation(userId, iJoint, flip);
        ...
        void MoveAvatar(uint UserID)
        {
            if (Root == null || Root.parent == null)
                return;
            if (!Manager.Instance.IsJointTracked(UserID, (int)Wrapper.SkeletonJoint.HIPS))
                return;

            // Get the position of the body and store it.
            Vector3 trans = Manager.Instance.GetUserPosition(UserID);
