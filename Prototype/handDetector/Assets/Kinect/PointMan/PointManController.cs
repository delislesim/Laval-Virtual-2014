...
public class PointManController : MonoBehaviour 
{
	...
	void Start () 
	{
		...
		_bones = new GameObject[(int)Wrapper.NuiSkeletonPositionIndex.Count] {
	...
	{
		// get 1st player
		uint playerID = Manager.Instance != null ? Manager.Instance.GetPlayer1ID() : 0;
		if(playerID <= 0)
			return;
		
		// set the position in space
		Vector3 posPointMan = Manager.Instance.GetUserPosition(playerID);
		...
		for(int i = 0; i < jointsCount; i++) 
		{
			if(_bones[i] != null)
			{
				if(Manager.Instance.IsJointTracked(playerID, i))
				{
					_bones[i].gameObject.SetActive(true);
					
					int joint = MirroredMovement ? Wrapper.GetSkeletonMirroredJoint(i): i;
					Vector3 posJoint = Manager.Instance.GetJointPosition(playerID, joint);
					posJoint.z = !MirroredMovement ? -posJoint.z : posJoint.z;
					Quaternion rotJoint = Manager.Instance.GetJointOrientation(playerID, joint, !MirroredMovement);
