...
namespace Kinect
{
    ...
    public class Manager : MonoBehaviour
    {
        ...
        public Wrapper.Gestures Player1CalibrationPose;
        public Wrapper.Gestures Player2CalibrationPose;

        // List of Gestures to detect for each player
        public List<Wrapper.Gestures> Player1Gestures;
        public List<Wrapper.Gestures> Player2Gestures;
        ...
        private Wrapper.NuiSkeletonFrame skeletonFrame;
        private Wrapper.NuiTransformSmoothParameters smoothParameters;
        ...
        private Wrapper.NuiSkeletonBoneOrientation[] jointOrientations;

        // Calibration gesture data for each player
        private Wrapper.GestureData player1CalibrationData;
        private Wrapper.GestureData player2CalibrationData;

        // Lists of gesture data, for each player
        private List<Wrapper.GestureData> player1Gestures = new List<Wrapper.GestureData>();
        private List<Wrapper.GestureData> player2Gestures = new List<Wrapper.GestureData>();
        ...
        {
            if (UserId == Player1ID && player1JointsTracked[(int)Wrapper.NuiSkeletonPositionIndex.HipCenter])
                return ConvertMatrixToQuat(player1Ori, (int)Wrapper.NuiSkeletonPositionIndex.HipCenter, flip);
            else if (UserId == Player2ID && player2JointsTracked[(int)Wrapper.NuiSkeletonPositionIndex.HipCenter])
                return ConvertMatrixToQuat(player2Ori, (int)Wrapper.NuiSkeletonPositionIndex.HipCenter, flip);
        ...
        {
            int parent = Wrapper.GetSkeletonJointParent(joint);
        ...
        public void DetectGesture(uint UserId, Wrapper.Gestures gesture)
        {
            int index = GetGestureIndex(UserId, gesture);
            if (index >= 0)
                DeleteGesture(UserId, gesture);

            Wrapper.GestureData gestureData = new Wrapper.GestureData();
            ...
            gestureData.checkForGestures = new List<Wrapper.Gestures>();
            switch (gesture)
            {
                case Wrapper.Gestures.ZoomIn:
                    gestureData.checkForGestures.Add(Wrapper.Gestures.ZoomOut);
                    gestureData.checkForGestures.Add(Wrapper.Gestures.Wheel);
                    break;

                case Wrapper.Gestures.ZoomOut:
                    gestureData.checkForGestures.Add(Wrapper.Gestures.ZoomIn);
                    gestureData.checkForGestures.Add(Wrapper.Gestures.Wheel);
                    break;

                case Wrapper.Gestures.Wheel:
                    gestureData.checkForGestures.Add(Wrapper.Gestures.ZoomIn);
                    gestureData.checkForGestures.Add(Wrapper.Gestures.ZoomOut);
                    break;
            }
        ...
        public bool ResetGesture(uint UserId, Wrapper.Gestures gesture)
        {
            int index = GetGestureIndex(UserId, gesture);
            if (index < 0)
                return false;

            Wrapper.GestureData gestureData = (UserId == Player1ID) ? player1Gestures[index] : player2Gestures[index];
            ...
            gestureData.complete = false;
            gestureData.cancelled = false;
            gestureData.startTrackingAtTime = Time.realtimeSinceStartup + Wrapper.Constants.MinTimeBetweenGestures;
        ...
        public bool DeleteGesture(uint UserId, Wrapper.Gestures gesture)
        {
            int index = GetGestureIndex(UserId, gesture);
            if (index < 0)
                return false;
        ...
        public List<Wrapper.Gestures> GetGesturesList(uint UserId)
        {
            List<Wrapper.Gestures> list = new List<Wrapper.Gestures>();

            if (UserId == Player1ID)
            {
                foreach (Wrapper.GestureData data in player1Gestures)
        ...
        public bool IsGestureDetected(uint UserId, Wrapper.Gestures gesture)
        {
            int index = GetGestureIndex(UserId, gesture);
            return index >= 0;
        }
        ...
        public bool IsGestureComplete(uint UserId, Wrapper.Gestures gesture, bool bResetOnComplete)
        {
            int index = GetGestureIndex(UserId, gesture);
            ...
            if (index >= 0)
            {
                if (UserId == Player1ID)
                {
                    Wrapper.GestureData gestureData = player1Gestures[index];
                ...
                {
                    Wrapper.GestureData gestureData = player2Gestures[index];
        ...
        public bool IsGestureCancelled(uint UserId, Wrapper.Gestures gesture)
        {
            int index = GetGestureIndex(UserId, gesture);
        ...
        public float GetGestureProgress(uint UserId, Wrapper.Gestures gesture)
        {
            int index = GetGestureIndex(UserId, gesture);
        ...
        public Vector3 GetGestureScreenPos(uint UserId, Wrapper.Gestures gesture)
        {
            int index = GetGestureIndex(UserId, gesture);
        ...
        public void ResetFilters()
        {
            ...
            int skeletonJointsCount = (int)Wrapper.NuiSkeletonPositionIndex.Count;
        ...
        void Start()
        {
            ...
            try
            {
                hr = Wrapper.NuiInitialize(Wrapper.NuiInitializeFlags.UsesDepthAndPlayerIndex |
                    Wrapper.NuiInitializeFlags.UsesSkeleton |
                    (ComputeColorMap ? Wrapper.NuiInitializeFlags.UsesColor : 0));
                if (hr != 0)
                {
                    throw new Exception("NuiInitialize Failed");
                }

                hr = Wrapper.NuiSkeletonTrackingEnable(IntPtr.Zero, 8);  // 0, 12,8
                if (hr != 0)
                {
                ...
                if (ComputeUserMap)
                {
                    hr = Wrapper.NuiImageStreamOpen(Wrapper.NuiImageType.DepthAndPlayerIndex,
                        Wrapper.Constants.ImageResolution, 0, 2, IntPtr.Zero, ref depthStreamHandle);
                    if (hr != 0)
                    {
                ...
                if (ComputeColorMap)
                {
                    hr = Wrapper.NuiImageStreamOpen(Wrapper.NuiImageType.Color,
                        Wrapper.Constants.ImageResolution, 0, 2, IntPtr.Zero, ref colorStreamHandle);
                    if (hr != 0)
                    {
                ...
                Wrapper.NuiCameraElevationSetAngle(SensorAngle);

                // init skeleton structures
                skeletonFrame = new Wrapper.NuiSkeletonFrame()
                                {
                                    SkeletonData = new Wrapper.NuiSkeletonData[Wrapper.Constants.NuiSkeletonCount]
                                };

                // values used to pass to smoothing function
                smoothParameters = new Wrapper.NuiTransformSmoothParameters();
                ...
                trackingStateFilter = new TrackingStateFilter[Wrapper.Constants.NuiSkeletonMaxTracked];
                for (int i = 0; i < trackingStateFilter.Length; i++)
                {
                ...
                boneOrientationFilter = new BoneOrientationsFilter[Wrapper.Constants.NuiSkeletonMaxTracked];
                for (int i = 0; i < boneOrientationFilter.Length; i++)
                {
                ...
                clippedLegsFilter = new ClippedLegsFilter[Wrapper.Constants.NuiSkeletonMaxTracked];
                for (int i = 0; i < clippedLegsFilter.Length; i++)
                {
                ...
                gestureTrackingAtTime = new float[Wrapper.Constants.NuiSkeletonMaxTracked];
            ...
            {
                string message = e.Message + " - " + Wrapper.GetNuiErrorString(hr);
                Debug.LogError(message);
                Debug.LogError(e.ToString());
                if (CalibrationText != null)
                    CalibrationText.guiText.text = message;
            ...
            if (ComputeUserMap)
            {
                ...
                usersMapSize = Wrapper.GetDepthWidth() * Wrapper.GetDepthHeight();
                usersLblTex = new Texture2D(Wrapper.GetDepthWidth(), Wrapper.GetDepthHeight());
            ...
            if (ComputeColorMap)
            {
                ...
                usersClrTex = new Texture2D(Wrapper.GetDepthWidth(), Wrapper.GetDepthHeight());
                ...
                colorImage = new Color32[Wrapper.GetDepthWidth() * Wrapper.GetDepthHeight()];
            ...
            if (KinectInitialized)
            {
                ...
                if (ComputeUserMap)
                {
                    if (depthStreamHandle != IntPtr.Zero &&
                        Wrapper.PollDepth(depthStreamHandle, NearMode, ref usersDepthMap))
                    {
                        UpdateUserMap();
                    }
                }
                ...
                if (ComputeColorMap)
                {
                    if (colorStreamHandle != IntPtr.Zero &&
                        Wrapper.PollColor(colorStreamHandle, ref usersColorMap, ref colorImage))
                    {
                        UpdateColorMap();
                    }
                }

                if (Wrapper.PollSkeleton(ref smoothParameters, ref skeletonFrame))
                {
                    ProcessSkeleton();
                }
                ...
                if (Player1Calibrated)
                {
                    foreach (AvatarController controller in Player1Controllers)
                    {
                        ...
                        foreach (Wrapper.GestureData gestureData in player1Gestures)
                        {
                            if (gestureData.complete)
                            {
                                if (controller.GestureComplete(Player1ID, gestureData.gesture,
                                    (Wrapper.SkeletonJoint)gestureData.joint, gestureData.screenPos))
                                {
                            ...
                            else if (gestureData.cancelled)
                            {
                                if (controller.GestureCancelled(Player1ID, gestureData.gesture,
                                    (Wrapper.SkeletonJoint)gestureData.joint))
                                {
                                    ResetGesture(Player1ID, gestureData.gesture);
                                }
                            }
                            ...
                            {
                                if ((gestureData.gesture == Wrapper.Gestures.RightHandCursor ||
                                    gestureData.gesture == Wrapper.Gestures.LeftHandCursor) &&
                ...
                if (Player2Calibrated)
                {
                    foreach (AvatarController controller in Player2Controllers)
                    {
                        ...
                        // Check for complete gestures
                        foreach (Wrapper.GestureData gestureData in player2Gestures)
                        {
                            if (gestureData.complete)
                            {
        ...
        void UpdateUserMap()
        {
            ...
            Wrapper.NuiImageViewArea pcViewArea = new Wrapper.NuiImageViewArea
            {
                eDigitalZoom = 0,
                lCenterX = 0,
                lCenterY = 0
            ...
            for (int i = 0; i < usersMapSize; i++)
            {
                ...
                if (nowUserPixel != wasUserPixel)
                {
                    ...
                    else
                    {
                        if (colorImage != null)
                        {
                            int x = i % Wrapper.Constants.ImageWidth;
                            int y = i / Wrapper.Constants.ImageWidth;

                            int cx, cy;
                            int hr = Wrapper.NuiImageGetColorPixelCoordinatesFromDepthPixelAtResolution(
                                Wrapper.Constants.ImageResolution,
                                Wrapper.Constants.ImageResolution,
                                ref pcViewArea,
                                x, y, usersDepthMap[i],
                                out cx, out cy);
                            ...
                            if (hr == 0)
                            {
                                int colorIndex = cx + cy * Wrapper.Constants.ImageWidth;
        ...
        void CalibrateUser(uint UserId, ref Wrapper.NuiSkeletonData skeletonData)
        {
            ...
            if (!Player1Calibrated)
            {
                ...
                if (!allUsers.Contains(UserId))
                {
                    if (CheckForCalibrationPose(UserId, ref Player1CalibrationPose, ref player1CalibrationData, ref skeletonData))
                    {
                        ...
                        foreach (Wrapper.Gestures gesture in Player1Gestures)
                        {
            ...
            else if (TwoUsers && !Player2Calibrated)
            {
                if (!allUsers.Contains(UserId))
                {
                    if (CheckForCalibrationPose(UserId, ref Player2CalibrationPose, ref player2CalibrationData, ref skeletonData))
                    {
                        ...
                        // add the gestures to detect, if any
                        foreach (Wrapper.Gestures gesture in Player2Gestures)
                        {
                            DetectGesture(UserId, gesture);
                        }
        ...
        private const int stateTracked = (int)Wrapper.NuiSkeletonPositionTrackingState.Tracked;
        private const int stateNotTracked = (int)Wrapper.NuiSkeletonPositionTrackingState.NotTracked;

        private int[] mustBeTrackedJoints = { 
		(int)Wrapper.NuiSkeletonPositionIndex.AnkleLeft,
		(int)Wrapper.NuiSkeletonPositionIndex.FootLeft,
		(int)Wrapper.NuiSkeletonPositionIndex.AnkleRight,
		(int)Wrapper.NuiSkeletonPositionIndex.FootRight,
	};
        ...
        void ProcessSkeleton()
        {
            ...
            for (int i = 0; i < Wrapper.Constants.NuiSkeletonCount; i++)
            {
                Wrapper.NuiSkeletonData skeletonData = skeletonFrame.SkeletonData[i];
                uint userId = skeletonData.dwTrackingID;

                if (skeletonData.eTrackingState == Wrapper.NuiSkeletonTrackingState.SkeletonTracked)
                {
                    if (!AllPlayersCalibrated)
                    {
                    ...
                    if (userId == Player1ID && skeletonPos.z >= MinUserDistance)
                    {
                        ...
                        for (int j = 0; j < (int)Wrapper.NuiSkeletonPositionIndex.Count; j++)
                        {
                        ...
                        Wrapper.GetSkeletonJointOrientation(ref player1JointsPos, ref player1JointsTracked, ref player1JointsOri);
                        ...
                        // get player rotation
                        player1Ori = player1JointsOri[(int)Wrapper.NuiSkeletonPositionIndex.HipCenter];
                        ...
                        if (Time.realtimeSinceStartup >= gestureTrackingAtTime[0])
                        {
                            ...
                            {
                                Wrapper.GestureData gestureData = player1Gestures[g];

                                if ((timestampNow >= gestureData.startTrackingAtTime) &&
                                    !IsConflictingGestureInProgress(gestureData))
                                {
                                    Wrapper.CheckForGesture(userId, ref gestureData, Time.realtimeSinceStartup,
                    ...
                    else if (userId == Player2ID && skeletonPos.z >= MinUserDistance)
                    {
                        ...
                        // calculate joint orientations
                        Wrapper.GetSkeletonJointOrientation(ref player2JointsPos, ref player2JointsTracked, ref player2JointsOri);
                        ...
                        // get player rotation
                        player2Ori = player2JointsOri[(int)Wrapper.NuiSkeletonPositionIndex.HipCenter];
                        ...
                        if (Time.realtimeSinceStartup >= gestureTrackingAtTime[1])
                        {
                            ...
                            for (int g = 0; g < listGestureSize; g++)
                            {
                                Wrapper.GestureData gestureData = player2Gestures[g];
        ...
        private void DrawSkeleton(Texture2D aTexture, ref Wrapper.NuiSkeletonData skeletonData, ref bool[] playerJointsTracked)
        {
            int jointsCount = (int)Wrapper.NuiSkeletonPositionIndex.Count;

            for (int i = 0; i < jointsCount; i++)
            {
                int parent = Wrapper.GetSkeletonJointParent(i);

                if (playerJointsTracked[i] && playerJointsTracked[parent])
                {
                    Vector3 posParent = Wrapper.MapSkeletonPointToDepthPoint(skeletonData.SkeletonPositions[parent]);
                    Vector3 posJoint = Wrapper.MapSkeletonPointToDepthPoint(skeletonData.SkeletonPositions[i]);

                    posParent.y = Wrapper.Constants.ImageHeight - posParent.y - 1;
                    posJoint.y = Wrapper.Constants.ImageHeight - posJoint.y - 1;
                    posParent.x = Wrapper.Constants.ImageWidth - posParent.x - 1;
                    posJoint.x = Wrapper.Constants.ImageWidth - posJoint.x - 1;
        ...
        {
            int width = Wrapper.Constants.ImageWidth;
            int height = Wrapper.Constants.ImageHeight;
        ...
        private int GetGestureIndex(uint UserId, Wrapper.Gestures gesture)
        {
            if (UserId == Player1ID)
            {
        ...
        private bool IsConflictingGestureInProgress(Wrapper.GestureData gestureData)
        {
            foreach (Wrapper.Gestures gesture in gestureData.checkForGestures)
            {
        ...
        private bool CheckForCalibrationPose(uint userId, ref Wrapper.Gestures calibrationGesture,
            ref Wrapper.GestureData gestureData, ref Wrapper.NuiSkeletonData skeletonData)
        {
            if (calibrationGesture == Wrapper.Gestures.None)
                return true;
            ...
            int stateTracked = (int)Wrapper.NuiSkeletonPositionTrackingState.Tracked;
            int stateNotTracked = (int)Wrapper.NuiSkeletonPositionTrackingState.NotTracked;
